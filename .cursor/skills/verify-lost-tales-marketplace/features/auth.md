...
- **Successful login.** Submit the seeded collector. Run `$VERIFY drive login --email collector.one@example.com --password replace-me-local-only`. Primary nav shows `Hi, Collector One` and a `Sign out` button. Default landing is `/collections`. Assert the heading with `$VERIFY drive expect --role heading --name "Your Collection" --exact` (`Bulk update your collection` also matches the substring).
- **Logout.** Sign out. Run `$VERIFY drive logout`. Primary nav shows `Sign in` again.
- **Gated redirect.** Choose Collection while signed out. Run `$VERIFY drive click --role link --name Collection --scope nav`. The login heading is visible (not Collection). After `$VERIFY drive login --email collector.one@example.com --password replace-me-local-only`, `$VERIFY drive expect --role heading --name "Your Collection" --exact`.
