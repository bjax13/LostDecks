...
describe("useTradeMatches cache + cooldown", () => {
  ...
  it("reads and writes user-keyed session cache", () => {
    ...
    expect(window.sessionStorage.getItem(getMatchesCacheKey("uid-1"))).toBeTruthy();
  ...
  it("treats malformed cache as a miss", async () => {
    window.sessionStorage.setItem(getMatchesCacheKey("uid-1"), "{not-json");
    const { result } = renderHook(() => useTradeMatches("uid-1"));
