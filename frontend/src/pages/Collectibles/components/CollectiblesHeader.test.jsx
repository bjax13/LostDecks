...
describe("CollectiblesHeader (unit)", () => {
  it("renders set name and total collectibles", () => {
    render(
      <CollectiblesHeader setName="Lost Tales" totalCollectibles={100} viewMode="grid" onChangeView={() => {}} />,
    );
  ...
  it("calls onChangeView with grid when Grid view clicked", async () => {
    ...
    render(
      <CollectiblesHeader setName="LT" totalCollectibles={50} viewMode="table" onChangeView={onChangeView} />,
    );
  ...
  it("calls onChangeView with table when Table view clicked", async () => {
    const onChangeView = vi.fn();
    render(
      <CollectiblesHeader setName="LT" totalCollectibles={50} viewMode="grid" onChangeView={onChangeView} />,
    );
  ...
  it("applies active class to current view mode button", () => {
    const { rerender } = render(
      <CollectiblesHeader setName="LT" totalCollectibles={50} viewMode="grid" onChangeView={() => {}} />,
    );
    expect(screen.getByRole("button", { name: "Grid view" })).toHaveClass("active");
    rerender(
      <CollectiblesHeader setName="LT" totalCollectibles={50} viewMode="table" onChangeView={() => {}} />,
    );
