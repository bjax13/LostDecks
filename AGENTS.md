...
2. **GitHub Actions (optional automation)**  
   ...
   **Free-tier survey cap (optional):** PostHog includes 1,500 survey responses/month. To hide the in-app FAB when the survey is no longer accepting responses, set the survey completion condition **Stop accepting responses after** ~1500 (or stop the survey manually). The FAB only renders when `getSurveys` still returns that survey as active (no past `end_date`). That limit is per survey lifetime, not per billing month—relaunch or clear the end date if you want the FAB back next period. Separately, set a Surveys **billing limit** of $0 in Organization → Billing so you never pay for overage even if a few responses slip through while PostHog processes the stop.
