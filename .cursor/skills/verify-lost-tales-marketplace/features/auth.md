...
- **Login page.** Open sign in. Run `$VERIFY drive click --role link --name "Sign in" --scope nav`. The heading `Sign in to ShardStash` is visible with Email and Password fields.
...
- **Gated redirect.** Choose Collection while signed out. Run `$VERIFY drive click --role link --name Collection --scope nav`. The login heading is visible (not Collection). Fill the same page (do not `$VERIFY drive login` unless you already are on this login view): `$VERIFY drive fill --label Email --value collector.one@example.com`, `$VERIFY drive fill --label Password --value replace-me-local-only`, `$VERIFY drive click --role button --name "Sign In" --exact`. Heading `Your Collection` appears; expect it with `$VERIFY drive expect --role heading --name "Your Collection" --exact`.
- **Quick sign in modal.** Sign out from a page that is not `/auth/login` so the modal heading is unique, or use `--exact`. Run `$VERIFY drive logout`, `$VERIFY drive goto --path /`, `$VERIFY drive click --role button --name "Quick sign in" --scope nav`. Heading `Sign In` is visible (modal h2). Close with `$VERIFY drive click --role button --name ×`.
...
- `$VERIFY drive login` fills `/auth/login` and presses `Sign In` with an exact name. It is the same user path as the labeled steps, not an Auth REST shortcut. When already on `/auth/login` from an AuthGuard redirect, that goto usually keeps `state.from` and returns to the gated page. From any other route it lands on `/`.
- Login page heading is `Sign in to ShardStash`. Modal heading is exactly `Sign In`. Submit on both is `Sign In`. Without `--exact`, `$VERIFY drive click --role button --name "Sign In"` also matches nav `Quick sign in`, and `$VERIFY drive expect --role heading --name "Sign In"` also matches the login page h1.
- `$VERIFY drive expect --role heading --name "Your Collection"` without `--exact` also matches `Bulk update your collection`.
