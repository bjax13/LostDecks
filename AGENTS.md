...
1. **Firebase console (one-time per project)**  
   ...
   - If that step fails (missing token or 403), add the domains by hand: Firebase console → project `storydeck-16` → Authentication → Settings → Authorized domains → Add domain. Google sign-in on the new host will fail until these domains are listed. Grant the GitHub Actions service account **Identity Toolkit Admin** (`roles/identitytoolkit.admin`) or **Editor** if you want CI to update them.
 - **Google OAuth web client (separate from Auth authorized domains):** Production builds set `VITE_FIREBASE_AUTH_DOMAIN` to the public Hosting host (`shardstash.web.app` via `scripts/productionAuthDomain.js`). If Google’s account chooser still says `storydeck-16.firebaseapp.com`, add `https://shardstash.web.app` and `https://shardstash.web.app/__/auth/handler` to the Firebase auto-created OAuth 2.0 web client in Google Cloud Console → project `storydeck-16` → APIs & Services → Credentials. `npm run deploy:hosting` prints the full URI checklist; `npm run verify:google-auth-host` checks the live chooser (requires Playwright Chromium in `frontend/`).  
...
2. **GitHub Actions (optional automation)**  
   ...
   | `VITE_POSTHOG_SURVEY_ID` | PostHog survey id for the in-app **Submit feedback** FAB (`posthog.displaySurvey`). Set after creating the survey in PostHog (see below). |
   ...
   **PostHog feedback survey (one-time in PostHog, then secret):** In PostHog → **Surveys** → **New survey** → template **Open feedback**. Suggested questions: optional single choice **Bug / Idea / Praise**, then open text **What's on your mind?** Set presentation to **API** or **Feedback button** (the app uses a custom FAB and calls `displaySurvey` on click; do not rely on PostHog's automatic tab). Leave display conditions loose or off—the FAB passes `ignoreConditions` / `ignoreDelay` so every click opens the survey. Copy the survey **ID** into the `VITE_POSTHOG_SURVEY_ID` repository secret (and `frontend/.env` for local testing). Optional: Surveys → your survey → **Notifications** to add a Slack or Teams webhook for new responses (configured in PostHog, not in this repo). The footer **Feedback** mailto/Discord modal remains as a secondary path.
